...
# SDTP config (should match Stage2)
MAX_NEW_TOKENS = 128
...
def load_model_and_pruners_flash():
    """
    Load Qwen2 with FlashAttention2 enabled and attach SDTP pruning modules.
    """
...
def prefill_with_pruning_flash(
    ...
    """
    Manual forward over all transformer layers with SDTP pruning.
...
def profile_lengths_flash(lengths, keep_ratio: float):
    ...
    print("[Flash SDTP] Profiling lengths:", lengths)
    for L in lengths:
        ...
        try:
            ...
            speed = base_t / sdtp_t if sdtp_t > 0 else float("inf")
            print(
                f"[Length {L}] baseline={base_t:.4f}s  "
                f"flash_sdtp={sdtp_t:.4f}s  speedup={speed:.2f}x"
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--prompt",
        type=str,
        default="Hello, SDTP with FlashAttention2!",
    )
